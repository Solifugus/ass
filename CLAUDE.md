...
Companion docs: `README.md` (usage/features), `COMPATIBILITY.md` (matrix + deferrals), `CONTRIBUTING.md` (how to extend), `ass-design.md` (rationale). The architecture notes below remain the conceptual map. Known deferrals (tracked in PLAN.md): column/pointer input, GLM CLASS effects, SAS-verified corpus output backfill (+ `--compare-output`/JSON). (`proc format` user-defined VALUE formats, two-way PROC FREQ cross-tabulation, PROC REG `Pr>|t|` p-values, list-input informats, and dataset options `keep=`/`drop=`/`rename=`/`where=` are now implemented — see the 2026-06-17 progress log.)
