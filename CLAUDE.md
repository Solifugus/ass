...
- `dbio/` — external-database LIBNAME engines (`table.Backend`/`table.WriteBackend` over `database/sql`); read + write for Postgres/SQL Server/Oracle (pure-Go drivers), SQLite (CGo), and DB2 (CGo + IBM CLI driver, `-tags db2`). See `docs/databases.md`
