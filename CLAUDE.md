...
Companion docs: `README.md` (usage/features), `COMPATIBILITY.md` (matrix + deferrals), `CONTRIBUTING.md` (how to extend), `ass-design.md` (rationale). The architecture notes below remain the conceptual map. Known deferrals (tracked in PLAN.md): two-way PROC FREQ, informats, dataset-option `where=`/`keep=`/`drop=`, `Pr>|t|`/GLM class effects, SAS-verified corpus output backfill (+ `--compare-output`/JSON). (`proc format` user-defined VALUE formats are now implemented — see the 2026-06-17 progress log.)
