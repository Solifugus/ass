...
Generated from the compatibility corpus via `ass test corpus/` (2026-06-20; FIRSTOBS=/OBS= options).
...
| Executed | 50 (100.0%) |
| Passed | 50 (100.0%) |
| Value-verified | 24 items assert dataset values; all match |
...
- Numbered var-list ranges in `keep=`/`drop=` (e.g. `keep=x1-x5`) and options on PROC `out=`. Supported dataset options on SET/MERGE/DATA/PROC `data=`: `keep=`/`drop=`/`rename=`/`where=` and **`firstobs=`/`obs=`** (positional observation range — `firstobs=` first observation, `obs=` last-observation number, applied before WHERE)
