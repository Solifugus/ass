...
- Still open (separate display gaps, not inheritance): PROC PRINT's `label` option does not render column labels (labels are now carried in metadata but not shown); and the cosmetic `_DATAOPT_`-prefixed NOTE when a PROC reads an external dataset with no dataset options.
