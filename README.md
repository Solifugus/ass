...
| Flat-file input | `infile "path"` with `dlm=`/`delimiter=`, `dsd` (CSV: quoted fields, embedded delimiters, missing), `firstobs=`, `obs=`, `lrecl=`, `pad`, `end=` (last-record flag); list, column (`1-10`), formatted, `@n`/`+n` pointer, and `#n` multi-line input |
| Flat-file output | `file "path"` with `dlm=`/`dsd` (CSV: quotes values containing the delimiter), `mod` (append); `put` of variables, string literals, formatted values, column/pointer placement (`name $ 1-10`, `@n`/`+n`), `#n` multi-line output, trailing `@`/`@@` output hold, `put _all_`, and named `var=` output |
...
| PROC FORMAT | user-defined `value` formats (ranges, `low`/`high`, `other`, char), applied in PROC PRINT and for grouping in PROC FREQ/MEANS; `invalue` user informats read by INPUT; `picture` output templates (digit selectors, message chars, `prefix=`/`mult=`/`fill=`); `cntlout=`/`cntlin=` (persist/rebuild VALUE formats as a control dataset) |
...
| Titles / footnotes | `title`/`title2`…`title10` shown above and `footnote`…`footnote10` below procedure output (bare `title;`/`footnote;` clears); persist across steps and notebook cells |
