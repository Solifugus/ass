...
Not yet supported (selected): informats, dataset options (`where=`/`keep=`/`rename=`), PROC FREQ n-way tables and association statistics, PROC REG/GLM significance probabilities (`Pr>|t|`) and GLM CLASS effects. See [`COMPATIBILITY.md`](COMPATIBILITY.md) and `corpus/FEATURES.md`.
